...
class BaseTrainer:
    ...
    def train(self, model, train_dataset, val_dataset=None):
        ...
        if self.config.use_gpu:
            model.to(mps_device)
            logger.info("Use Apple Silicon MPS")
        ...
        for epoch in range(init_epoch, max_epochs):
            ...
            for batch_train in train_loader:
                ...
                batch_inputs = self._move_inputs_to_device(batch_inputs)
                if self.config.use_gpu:
                    batch_labels = batch_labels.to(mps_device)

                batch_outputs = self._forward_model(model, batch_inputs)
                if isinstance(self.loss_fn, CrossEntropyLoss):
                    targets = batch_labels.argmax(dim=1).long()
                    batch_loss = self.loss_fn(input=batch_outputs, target=targets)
                else:
            ...
            if val_dataset:
                ...
                if isinstance(self.loss_fn, CrossEntropyLoss):
                    val_targets = val_labels.argmax(dim=1).long()
                    val_loss = self.loss_fn(input=val_outputs, target=val_targets)
                    val_prob = torch.softmax(val_outputs, dim=1).numpy()
                    pred_idx = np.argmax(val_prob, axis=1)
                    val_pred = np.zeros_like(val_prob)
                    val_pred[np.arange(val_pred.shape[0]), pred_idx] = 1
                else:
    ...
    def test(self, model, test_dataset):
        ...
        if self.config.use_gpu:
            model.to(mps_device)
            logger.info("Use Apple Sillicon MPS")
    ...
    def _compute_metrics(self, outputs, labels, metric_names=None):
        ...
        if isinstance(self.loss_fn, CrossEntropyLoss):
            probs = np.array(torch.softmax(outputs, dim=1))
        ...
        for metric_name in metric_names:
            if metric_name == "loss":
                if isinstance(self.loss_fn, CrossEntropyLoss):
                    ...
                    metric_vals["loss"] = self.loss_fn(input=outputs, target=targets)
                else:
    ...
    def _move_inputs_to_device(self, batch_inputs):
        if not self.config.use_gpu:
            return batch_inputs
        if torch.is_tensor(batch_inputs):
            return batch_inputs.to(mps_device)
        if isinstance(batch_inputs, dict):
            return {
                key: value.to(mps_device) if torch.is_tensor(value) else value
                for key, value in batch_inputs.items()
            }
        return batch_inputs

    def _forward_model(self, model, batch_inputs):
        if isinstance(batch_inputs, dict):
